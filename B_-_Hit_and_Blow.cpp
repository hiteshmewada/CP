...
void solve()
{
    ...
    vl v(n),vec(n);
    set<ll>st1,st2;
    rep(i,0,n) {cin>>v[i];st1.insert(v[i]);}
    rep(i,0,n) {cin>>vec[i];st2.insert(vec[i]);}
    vl ans,ans2;
    rep(i,0,n){
        if(v[i]==vec[i]) ans.pb(v[i]);
        else if(st1.find(vec[i])!=st1.end()) ans2.pb(vec[i]);
        // else if(st1.find(v[i])!=st2.end()) ans2.pb(v[i]);
    }
