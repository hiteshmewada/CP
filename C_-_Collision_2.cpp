...
void solve()
{
    ...
    cin>>n;
    vpl v;
    vector<pair<ll,pair<char,ll>>> vec;
    rep(i,0,n){
        cin>>a>>b;
        v.pb({a,b});
    }
    ...
    i=0;
    for(auto x:v){
        vec.pb({x.ss,{s[i],x.ff}});
        i++;
    }
    // sort(all(vec),[&](const pair<ll,char>&a,const pair<ll,char>&b){
    //     if(a.ff==b.ff){
    //         if(a.ss!=b.ss){
    //             // cout<<"No";
    //             k=1;
    ...
    rep(i,1,vec.size()){
        if(vec[i].ff==vec[i-1].ff and vec[i].ss.ff!=vec[i-1].ss.ff){
            if(vec[i].ss.ff=='R' and vec[i-1].ss.ss>vec[i].ss.ss){

            cout<<"Yes";return;
            }
            if(vec[i].ss.ff=='L' and vec[i-1].ss.ss<vec[i].ss.ss){
